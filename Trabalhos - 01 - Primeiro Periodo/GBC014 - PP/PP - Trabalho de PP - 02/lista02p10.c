...
int marcar(int campo_num[][100], char campo_most[][100], int lin, int col)
{
    ...
    printf("Qual a casa que voce deseja marcar ou revelar?\n");
    do
    {
    ...
    fflush(stdin);
    printf("\nVoce deseja marcar ou revelar a posicao (%d,%d)? (Digite M para marcar ou R para revelar): ",lin_marc,col_marc);
    scanf("%c",&opcao);
    fflush(stdin);
    opcao = toupper(opcao);
...
void revelar(int campo_num[][100], char campo_most[][100], int lin, int col, int lin_marc, int col_marc)
{
    ...
    if(campo_num[lin_marc][col_marc] == 0) campo_most[lin_marc][col_marc] = ' ';
