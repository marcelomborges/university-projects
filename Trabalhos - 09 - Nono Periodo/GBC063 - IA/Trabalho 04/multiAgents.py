...
class ReflexAgent(Agent):
    ...
    def getAction(self, gameState):
        ...
        """
        # Pega os movimentos possivel do estado atual
        legalMoves = gameState.getLegalActions()

        # Escolha a melhor acao
        scores = [self.evaluationFunction(gameState, action) for action in legalMoves]
        bestScore = max(scores)
        bestIndices = [index for index in range(len(scores)) if scores[index] == bestScore]
        chosenIndex = random.choice(bestIndices) # Pega um randomicamente entre os melhores
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        for action in gameState.getLegalActions(0):
            ...
            nextValue = self.pegarScore(nextState, 0, 1)
            if nextValue > maxValue:
                maxValue = nextValue
                maxAction = action
        return maxAction

    def pegarScore(self, gameState, currentDepth, agentIndex):
        if currentDepth == self.depth or gameState.isWin() or gameState.isLose():
            return self.evaluationFunction(gameState)
        elif agentIndex == 0:
            maxValue = float("-inf")
            for action in gameState.getLegalActions(0):
                maxValue = max(maxValue, self.pegarScore(gameState.generateSuccessor(0, action), currentDepth, 1))
            return maxValue
        else:
            minValue = float("inf")
            for action in gameState.getLegalActions(agentIndex):
                if agentIndex == gameState.getNumAgents()-1:
                    minValue = min(minValue, self.pegarScore(gameState.generateSuccessor(agentIndex, action), currentDepth+1, 0))
                else:
                    minValue = min(minValue, self.pegarScore(gameState.generateSuccessor(agentIndex, action), currentDepth, agentIndex+1))
            return minValue
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState): #retorna a melhor a ação a ser tomada após calcuar o maxScore
        maxScore = float("-inf")
        maxAction = Directions.STOP
        for action in gameState.getLegalActions(0):
            nextState = gameState.generateSuccessor(0, action)
            nextScore = self.getScore(nextState, 0, 1)
            if nextScore > maxScore:
                maxScore = nextScore
                maxAction = action
        return maxAction

    def getScore(self, gameState, currentDepth, agentIndex):
        if currentDepth == self.depth or gameState.isWin() or gameState.isLose():
            return self.evaluationFunction(gameState)
        elif agentIndex == 0:
            maxScore = float("-inf")
            for action in gameState.getLegalActions(0):
                maxScore = max(maxScore, self.getScore(gameState.generateSuccessor(0, action), currentDepth, 1))
            return maxScore
        else:
            avgScore = 0
            for action in gameState.getLegalActions(agentIndex):
                if agentIndex == gameState.getNumAgents()-1:
                    avgScore = avgScore + self.getScore(gameState.generateSuccessor(agentIndex, action), currentDepth+1, 0)
                else:
                    avgScore = avgScore + self.getScore(gameState.generateSuccessor(agentIndex, action), currentDepth, agentIndex+1)
            return avgScore

    #def maxScore(self, gameState, currentDepth):
        

    #def avgScore(self, gameState, currentDepth, agentIndex):
