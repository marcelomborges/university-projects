...
def play_maze(maze_obj, limit,tinkle):
	...
	while not (move > limit) and not maze_obj.is_done() and (sair == 0):
		action = gbc063.algoritmo(current, options)

		time.sleep(tinkle)
		info = maze_obj.move(action)

		#isso garante que o algoritmo pare caso encontre uma posição otima local
		#se action for igual ao current significa que não tem mais para onde ir
		if action == current:
			sair = 1
	...
	elif (move < limit) and (current != (9,9)):
		print('-> Distancia ate o objetivo: {0}'.format(distancia))
...
def main():
	...
	# set opcoes
	clock = 0.1
	seed = random.random()*10000 #7281.616623896421 #(seed no qual essa busca alcança o objetivo)		
	width = 10 	#20
	height = 10 	#12
	limite = 1000
	is_block = True
